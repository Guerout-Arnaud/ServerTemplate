...
int client_init(connection_t *client_info)
{
    ...
    log_msg(logger, LOG_INFO, asprintf(&logger->msg, "Connected to %s:%d\n", client_info->ip_addr, client_info->port));
    ...
    log_msg(logger, LOG_INFO, asprintf(&logger->msg, "Epoll passed.\n"));
        ...
        for (int i = 0; i < nfds; i++)
        {
            ...
            if (events[i].data.fd == client_info->signal_fd)
            {
                ...
                if (rcv_signal.ssi_signo == SIGINT || rcv_signal.ssi_signo == SIGQUIT)
                {
                    ...
                    log_msg(logger, LOG_INFO, asprintf(&logger->msg, "Client shutdown asked through signal\n"));
                    break;
                }
            }
            /* ToDo : THREAD THIS */
            ...
            else
            {
                ...
                {
                    log_msg(logger, LOG_ERROR, asprintf(&logger->msg, "Server disconnected: %s\n", (events[i].events & EPOLLERR) ? "EPOLLERR" : "EPOLLHUP"));
                    client_close(client_info);
                    return (ERROR);
                }
                if (events[i].events & EPOLLIN)
                {
                    ...
                    {
                        log_msg(logger, LOG_INFO, asprintf(&logger->msg, "%s\n", msg->content));
